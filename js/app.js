...
function renderPreacherPage(app, preacher) {
  ...
  sorted.forEach((s) => {
    ...
    if (s.summaryPdf) {
      ...
      a.className = 'action-btn';
      a.href = s.summaryPdf;
      a.setAttribute('download', s.summaryPdfName || '');
      a.innerHTML = iconFile() + ' Скачать конспект (PDF)';
      a.addEventListener('click', e => e.stopPropagation());
      actions.appendChild(a);
    }
    ...
    // --- text / summary tabs ---
    const hasSummary = s.summary && s.summary.length > 0;

    function blocksToHtml(blocks) {
      let html = '';
      blocks.forEach(block => {
        if (block.type === 'h') html += '<h3>' + esc(block.text) + '</h3>';
        else html += '<p>' + esc(block.text) + '</p>';
      });
      return html;
    }
    ...
    if (hasSummary && hasText) {
      ...
      tabFull.type = 'button';
      tabFull.className = 'content-tab';
      tabFull.textContent = 'Полный текст';

      const tabSummary = document.createElement('button');
      tabSummary.type = 'button';
      tabSummary.className = 'content-tab active';
      tabSummary.textContent = 'Конспект';
      ...
      summaryDiv.className = 'sermon-text';
      summaryDiv.innerHTML = blocksToHtml(s.summary);

      const fullDiv = document.createElement('div');
      fullDiv.className = 'sermon-text';
      fullDiv.style.display = 'none';
      fullDiv.innerHTML = blocksToHtml(s.text);

      bodyInner.appendChild(summaryDiv);
      bodyInner.appendChild(fullDiv);

      const switchTab = (activeBtn, activeDiv, inactiveBtn, inactiveDiv) => {
      ...
      tabFull.addEventListener('click', e => {
        e.stopPropagation();
        switchTab(tabFull, fullDiv, tabSummary, summaryDiv);
      });
      tabSummary.addEventListener('click', e => {
        e.stopPropagation();
        switchTab(tabSummary, summaryDiv, tabFull, fullDiv);
      });
    } else if (hasSummary || hasText) {
      const textDiv = document.createElement('div');
      textDiv.className = 'sermon-text';
      textDiv.innerHTML = blocksToHtml(hasText ? s.text : s.summary);
